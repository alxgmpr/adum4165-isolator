...
path=ROOT/'hub.kicad_pro';j=json.loads(path.read_text());base=deepcopy(j['net_settings']['classes'][0]);base.update(clearance=.15,track_width=.2,diff_pair_width=.15,diff_pair_gap=.15,diff_pair_via_gap=.25,via_diameter=.6,via_drill=.3)
classes=[base]
for priority,name,width in [(0,'USB90',.15),(1,'Power',1.0),(2,'Primary',.2),(3,'Secondary',.2)]:
...
rules=j['board']['design_settings']['rules'];rules.update(min_clearance=.15,min_track_width=.15,min_copper_edge_clearance=.5,min_silk_clearance=.1)
